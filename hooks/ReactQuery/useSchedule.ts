...
export const usePhases = ({ projectId }: { projectId: string }) => {
  return useQuery<Phase[], ApiError>({
    ...
    queryFn: async () => {
      ...
      const response = await api.get<PaginationResponse<Phase>>(
        API_ENDPOINTS.GET_PHASES,
        {
          params: queryParams,
        }
      );
...
export const useUpdatePhase = () => {
  ...
  return useMutation<
    APISuccessResponse,
    ApiError,
    { id: string; data: Partial<CreatePhaseDTO> }
  >({
...
export const useUpdateActivity = () => {
  ...
  return useMutation<
    APISuccessResponse,
    ApiError,
    { id: string; data: ActivityFormData }
  >({
...
export const useGetActivity = (params?: {
  ...
  return useQuery<PaginationResponse<Activity>>({
    queryKey: [
      ...
      params?.projectId,
      params?.phaseId,
      params?.includeProductionPlanning,
    ],
    queryFn: async () => {
      const queryParams = new URLSearchParams();
...
export const useUpdateWorkingDaysConfig = () => {
  ...
  return useMutation<
    APISuccessResponse,
    ApiError,
    { id: string; data: WorkingDaysConfig }
  >({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({
        queryKey: [QUERY_KEYS.WORKING_DAYS_CONFIG],
      });
    },
    onError: (error) => {
      toast.error(
        error.response?.data.message || "Failed to update working days config"
      );
    },
  });
};
...
export const useUpdateEquipment = () => {
  ...
  return useMutation<
    APISuccessResponse,
    ApiError,
    { id: string; data: Partial<CreateEquipmentDTO> }
  >({
...
export const useUpdateLabourers = () => {
  ...
  return useMutation<
    APISuccessResponse,
    ApiError,
    { id: string; data: Partial<CreateLabourerDTO> }
  >({
...
export const useCreateProductionPlanning = () => {
  ...
  return useMutation<APISuccessResponse, ApiError, any>({
    ...
    onError: (error) => {
      toast.error(
        error.response?.data.message || "Failed to create production planning"
      );
    },
  });
};
...
export const useUpdateProductionPlanning = () => {
  ...
  return useMutation<APISuccessResponse, ApiError, { id: string; data: any }>({
    ...
    onError: (error) => {
      toast.error(
        error.response?.data.message || "Failed to update production planning"
      );
    },
  });
};
...
export const useDeleteProductionPlanning = () => {
  ...
  return useMutation<APISuccessResponse, ApiError, string>({
    ...
    onError: (error) => {
      toast.error(
        error.response?.data.message || "Failed to delete production planning"
      );
    },
  });
};
...
export const useGetScheduleTracker = (params?: {
  cursor: string;
  limit: number;
  search: string;
  phaseId: string;
  ...
  return useQuery<PaginationResponse<ScheduleTracker>>({
    queryKey: [...QUERY_KEYS.SCHEDULE_TRACKER, params],
    queryFn: () => activityTracker(params ?? {}),
    // enabled: !!params?.projectId,
  });
};

        ...
        <div>
            <nav className="text-sm text-gray-500 mb-2" aria-label="Breadcrumb">
                <ol className="list-none p-0 inline-flex space-x-2">
                    ...
                    <li className="flex items-center">
                        <span className="text-gray-700 font-medium">{isEditMode ? 'Edit Role' : 'Create Role'}</span>
                    </li>
                </ol>
            </nav>
        </div>
        <div className='flex flex-col'>
            <h1 className="text-2xl font-semibold">{isEditMode ? 'Edit Role' : 'Create New Role'}</h1>
            <p className="text-gray-600">{isEditMode ? 'Update role details and permissions' : 'Define a role name and select permissions for modules'}</p>
        </div>
        <RoleCreation />
    </div>
  )
